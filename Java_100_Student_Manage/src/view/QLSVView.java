...
public class QLSVView extends JFrame {
	...
	public QLSVView() {
		...
		label_HoVaTen_2.setBounds(324, 472, 85, 23);
		...
		textField_Mon1.setBounds(438, 469, 166, 29);
		...
		label_HoVaTen_2_1.setBounds(324, 512, 85, 23);
		...
		textField_Mon3.setBounds(438, 553, 166, 29);
	...
	public void themHoacCapNhatThiSinh(ThiSinh ts) {
		...
		if (!this.model.kiemTraTonTai(ts)) {
			...
			// hiển thị thông báo khi có sv trùng mã sinh viên

			JOptionPane.showMessageDialog(new JFrame(), "Thí sinh đã tồn tại: ");
...
//		@SuppressWarnings("deprecation")
	...
	public void thucHienTim() {
		...
		Set<Integer> idCuaSinhVienCanTim = new TreeSet<Integer>();
		if (queQuan >= 0) {
			...
			for (int i = 0; i < soLuongDong; i++) {
				...
				if (!tenTinh.equals(tinhDaChon.getTenTinh())) {
					idCuaSinhVienCanTim.add(Integer.valueOf(id));
				}
			}
		}
		...
		for (Integer idCanXoa : idCuaSinhVienCanTim) {
//			System.out.println(idCanXoa);
	...
	public void thoatKhoiChuongTrinh() {
		int luaChon = JOptionPane.showConfirmDialog(this, "Bạn có muốn thoảt khỏi chương trình?", "Exit",
